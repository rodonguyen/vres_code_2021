...
def lr_readline(dataset_length,num_files):
    for i in range(1,num_files+1):
        filename = "./linear_regression_readline/code_length_%d/lr_ds%02d.py" % (dataset_length, i)
        f = open(filename, "w") 
        f.write(                          
...
def lr_readline_increase(num_files):
    for i in range(0,num_files+1):
        n = dataset_length[i]
        filename = "./linear_regression_readline/code_length_increase/lr_ds%02d.py" % (i)
        f = open(filename, "w") 
        f.write(                      
"""from sklearn import linear_model
